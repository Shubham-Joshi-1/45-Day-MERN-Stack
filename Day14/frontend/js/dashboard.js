 const URL = "http://localhost:3000/api/posts";
        const loggedInUser = localStorage.getItem("username") || "John Doe"; // auto-author
        ...
        async function fetchPosts() {
            try {
                ...
                if (!data.success || !data.data || data.data.length === 0) {
                    postsDiv.innerHTML = '<div class="no-posts">No posts found. Create your first post!</div>';
                    return;
                }
                ...
                posts.forEach(post => {
                    ...
                    div.innerHTML = `
                        ...
                        <div class="post-meta">
                            ...
                            <div class="tags">
                                <strong>Tags:</strong> ${post.tags?.length ? post.tags.map(t => `<span class="tag">${escapeHtml(t)}</span>`).join("") : '<span style="color: #a0aec0; font-style: italic;">No tags</span>'}
                            </div>
                            <p style="margin: 10px 0; display: flex; align-items: center;">
                                <strong style="margin-right: 10px;">Status:</strong> 
                                <span class="status-badge ${post.published ? "status-published" : "status-draft"}">${post.published ? "Published" : "Draft"}</span>
                            </p>
                            <p style="color: #718096; font-size: 12px; margin-bottom: 5px;">
                                <strong>📅 Created:</strong> ${new Date(post.createdAt).toLocaleString()}
                            </p>
                            ${post.updatedAt && post.updatedAt !== post.createdAt ? `<p style="color: #718096; font-size: 12px; margin-bottom: 0;"><strong>🔄 Updated:</strong> ${new Date(post.updatedAt).toLocaleString()}</p>` : ""}
                        </div>
                        <div class="actions">
                ...
                document.getElementById("posts").innerHTML = '<div class="error">Failed to load posts. Please check if the server is running.</div>';
            }
        }
        ...
        document.getElementById("postForm").addEventListener("submit", async (e) => {
            ...
            const postData = { title, content, tags, published, author: loggedInUser };
